...
type ChatMessageWithProfile = SessionChat & {
  profiles?: Pick<Profile, "full_name" | "email"> | null;
};

type PollWithOptions = Omit<SessionPoll, "options"> & {
  options: { id: string; text: string }[];
};

type PollResponseTyped = Omit<SessionPollResponse, "selected_options"> & {
...
export function SessionLobby({ sessionId }: SessionLobbyProps) {
  ...
  const loadMyPollResponses = useCallback(async () => {
    if (!user?.id) return;

    const pollResponsesRes = await supabase
      .from("session_poll_responses")
      .select("*")
      .eq("user_id", user.id);

    const pollResponsesData = (
      pollResponsesRes as unknown as { data: SessionPollResponse[] | null }
    ).data;
    if (pollResponsesData) {
      const typedResponses: PollResponseTyped[] = pollResponsesData.map(
        (r) => ({
          ...r,
          selected_options: (r.selected_options as unknown as string[]) ?? [],
        })
      );
      const responsesMap = new Map(typedResponses.map((r) => [r.poll_id, r]));
  ...
  }, [sessionId, loadChatMessages, loadPolls, loadMyPollResponses]);
  ...
  const handleCreatePoll = async () => {
    if (!user?.id || !pollQuestion.trim()) return;
    ...
    const { error } = await supabase.from("session_polls").insert({
      ...
      options: optionsArray as unknown as never,
      is_active: true,
  ...
  const handleVote = async (
    ...
    if (selectedOptions.length === 0) {
      ...
      await supabase.from("session_poll_responses").upsert({
        poll_id: pollId,
        user_id: user.id,
        selected_options: selectedOptions as unknown as never,
      });
    }
  ...
  const getPollResults = (poll: PollWithOptions) => {
    const responses = Array.from(pollResponses.values()).filter(
      (r) => r.poll_id === poll.id
    );
    const totalVotes = responses.reduce(
      (sum, r) => sum + r.selected_options.length,
      0
    );

    return poll.options.map((option) => {
      const votes = responses.filter((r) =>
        r.selected_options.includes(option.id)
      ).length;
          ...
          <div className="space-y-4">
            {polls.length === 0 ? (
              ...
              polls.map((poll) => {
                ...
                const hasVoted =
                  myResponse && myResponse.selected_options.length > 0;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      {results.map((option) => {
                        const isSelected =
                          myResponse?.selected_options.includes(option.id) ||
                          false;
