...
import { Calendar, Plus, Clock, MapPin, Users, CheckCircle, XCircle, Grid, List, Video } from 'lucide-react';
...
export function SessionsList() {
  ...
  const loadSessions = async () => {
    ...
    if (participations) {
      let sessionsData = participations
        .map((p: any) => p.study_sessions)
        .filter(Boolean);

      if (activeView === 'upcoming') {
        sessionsData = sessionsData.filter((s: Session) => new Date(s.scheduled_for) >= new Date(now));
        sessionsData.sort((a: Session, b: Session) => new Date(a.scheduled_for).getTime() - new Date(b.scheduled_for).getTime());
      } else {
        sessionsData = sessionsData.filter((s: Session) => new Date(s.scheduled_for) < new Date(now));
        sessionsData.sort((a: Session, b: Session) => new Date(b.scheduled_for).getTime() - new Date(a.scheduled_for).getTime());
      }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">Schedule and join study sessions</p>
        </div>
      ...
      {viewMode === 'calendar' ? (
        ...
        <>
          <div className="flex gap-2 mb-6 border-b border-gray-200 dark:border-gray-700">
            ...
            >
              Upcoming ({sessions.filter((s) => activeView === 'upcoming').length})
            </button>
            <button
      ...
      {sessions.length === 0 ? (
        <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 transition-colors">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            {activeView === 'upcoming' ? 'Schedule a session to get started' : 'Your past sessions will appear here'}
          </p>
          {activeView === 'upcoming' && (
            <button
              onClick={() => setShowCreateModal(true)}
          ...
          {sessions.map((session) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <h3 className="font-semibold text-gray-900 dark:text-white text-lg flex-1">{session.title}</h3>
              ...
              <div className="space-y-2 mb-4">
                ...
                <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <span>{formatDateTime(session.scheduled_for)}</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                {session.meeting_link && (
                  <div className="flex items-center gap-2 text-sm text-blue-600 dark:text-blue-400">
                    ...
                    <span className="truncate capitalize">
                      {session.meeting_platform?.replace('_', ' ') || 'Online Meeting'}
                    </span>
                  </div>
                )}
              </div>
