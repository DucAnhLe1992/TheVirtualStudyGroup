...
export function QuizzesList() {
  ...
  const [activeView, setActiveView] = useState<'available' | 'my-quizzes'>('available');
  ...
  const loadData = async () => {
    ...
    if (groupsData) {
      const groups = groupsData.map((m: any) => m.study_groups).filter(Boolean);
      setGroups(groups);

      const userGroupIds = groups.map((g: Group) => g.id);
      ...
      const { data: quizzesData } = await query;
  ...
  const handleDeleteQuiz = async (quizId: string) => {
    if (!confirm('Delete this quiz? This will also delete all questions and attempts.')) return;
    ...
    return (
      <div>
        <div className="mb-6">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">Test your knowledge</p>
        </div>
        ...
        <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 transition-colors">
          ...
          <p className="text-gray-600 dark:text-gray-400">Join a group to access quizzes</p>
        </div>
      </div>
    );
  }
            ...
            return (
              ...
              >
                ...
                <div className="flex gap-2 pt-3 border-t border-gray-200 dark:border-gray-700">
                  {!isOwner && quiz.questions_count! > 0 && (
                    <button
                      onClick={() => {
